...
## GitHubとは
---
複数人で[ソースコード](#_1)を編集できる、共同開発のための後述する様々な機能を持ったサービスです。<br>
しかも基本無料となっているのに加え、<br>
[ローカル環境を整備しなくとも](#_1)使うことができます！<br>
...
<dt>quaint-docs</dt>
<dd>この星陵祭オンライン整理券サイトの中身です。多くのファイルが .md 形式で、1つ1つが1ページに対応しています。</dd>
<dt>general</dt>
...
また、この検索は結構厳しく、部分一致ではヒットしないので気を付けてください。<br>
<br>
...
次に、[pages/index.vue](#_1)を例にして、ファイルの編集画面を説明します。<br>
...
mainは[seiryofes.com](#_1)の中身にあたります。<br>
...
![](images/github-6.png)<br>
この赤枠部分を押します。<br>

![](images/github-7.png)<br>
そうすると、このように最初から<br>
(issueの番号)-(issueの題名)<br>
という名前でつくることができます。<br>

![](images/github-8.png)<br>
...
また、誰もいないときにはassign yourselfというテキストがあるので、そちらを押すと自分自身をアサインできます。<br>
...
開いて少し待つと、VSCodeの画面が開きます。<br>
これがエクスプローラー、これがエディター、これがコンソール、といった風に説明します。<br>

また、設定でVSCodeのアプリが開くようにもできます。<br>
最初にVSCodeをインストールします（説明を省くか要検討）。<br>
ここの設定で変えられます（要加筆）。<br>

アプリ版の場合、127.0.0.1:3000からlocalhost:3000と入力し変更しましょう。<br>
そうすることでログイン処理ができます(どこで指定しているのか要加筆)<br>

* 最初にyarnを入れる(終了コード127)
* yarn run devあるいはエクスプローラーのdev
* yarn run lintfixあるいはエクスプローラーlintfix
(lintエラーを紹介)
...
GitHub ActionsやDependabotなどについて説明する予定です。<br>
また、学割についても説明を入れたいです。
...
* **pages/index.vue** … (quaint-appレポジトリ内の)pagesフォルダの中のindex.vueファイルという意味。<br>
このようなファイルの位置の示しかたはpathともいう。
